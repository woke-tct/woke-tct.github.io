...
window.setInterval(function () {
  if (
    ...
    campaignTrail_temp.candidate_json =
      campaignTrail_temp.candidate_json.filter((n) => n);
  }
}, 200);
...
campaignTrail_temp.margin_format =
  window.localStorage.getItem("margin_form") ?? "#FFFFFF";
...
!(function () {
  ...
  const shining_menu = (polling, e = campaignTrail_temp) => {
    ...
    $("#add_ad_spending").click(() => {
      ...
      let currentState = e.shining_data.ad_spending.find(
  ...
  const shining_cal = (polling) => {
    ...
    if (
      ...
      1 == e.election_json.find((f) => f.pk === e.election_id).fields.has_visits
    ) {
      const active_opps = e.opponents_list.filter(
        (f) =>
          e.candidate_json.find((_f) => _f.pk === f).fields.is_active === 1,
      );
      ...
      active_opps.forEach((f) => {
        ...
        const target = e.candidate_state_multiplier_json.find(
          (_f) =>
            _f.fields.state === closests[f].state && _f.fields.candidate === f,
        );
    ...
    our_info.lobby.forEach((f) => {
      let relevant = e.answer_score_issue_json.filter(
        (_f) =>
          _f.fields.issue === f.issue_tie &&
    ...
    media_boost =
      e.shining_data.balance < 0 ? media_boost - 0.0025 : media_boost; // debt penalty
    ...
    e.shining_data.ad_spending.forEach((f) => {
      const target = e.candidate_state_multiplier_json.find(
        (_f) =>
          f.state === _f.fields.state && _f.fields.candidate === e.candidate_id,
      );
...
e.max_answers = 6;
  ...
  function electionNight() {
    ...
    for (var t = 0; t < e.final_state_results[0].result.length; t++)
    ...
    if (id != 16) {
      if (modded) {
        try {
          ...
          yearbit =
            ree.election_json[findFromPK(ree.election_json, id)].fields.year;
          lastnamebit =
            ree.candidate_json[
  ...
  function s(t, i, l) {
    ...
    for (difficultyStr = "", r = 0; r < e.difficulty_level_json.length; r++)
      "Normal" == e.difficulty_level_json[r].fields.name
        ? (difficultyStr +=
            "<option value=" +
    ...
    $("#opponent_selection_id_button").click(function () {
      ...
      for (r = 0; r < e.opponents_default_json[d].candidates.length; r++) {
        e.opponents_default_json[d].candidates[r] != i &&
          -1 == a.indexOf(e.opponents_default_json[d].candidates[r]) &&
      ...
      (e.difficulty_level_multiplier =
        e.difficulty_level_json[d].fields.multiplier),
        (starting_mult =
          encrypted + e.difficulty_level_json[d].fields.multiplier);
      ...
      ((t, i, a, l, o) => {
        if (campaignTrail_temp.musicOn) {
          ...
          document.getElementById("campaigntrailmusic").src =
        ...
        if (modded == false) {
          ...
          (e.question_number = 0),
            ...
            (e.running_mate_issue_score_json =
  ...
  mapCache = function (skip = false) {
    ...
    if (!skip) {
      ...
      if (
        (e.question_number - 1) % 2 != 0 &&
        1 == e.election_json[i].fields.has_visits
      ) {
        return false;
      }
      if (
  ...
  function openMap(_e) {
    ...
    if ($("#main_content_area")[0]) {
      ...
      let game_window = $("#game_window")[0];
      let footer_html =
        '<button id="resume_questions_button">Back to the game</button><button id="margin_switcher">Switch margin colouring gradient</button><button id="AdvisorButton">Enable/Disable Advisor Feedback</button></div>';
    ...
    $("#AdvisorButton").click(function () {
      campaignTrail_temp.answer_feedback_flg =
        AdvisorFeedbackArr[campaignTrail_temp.answer_feedback_flg];
      o(_e);
    });
  ...
  function getLatestRes(t) {
    ...
    const filteredCandidates = e.candidate_json.filter(
      (candidate) =>
        containsObject(candidate.pk, e.opponents_list) ||
    ...
    filteredCandidates.forEach((candidate) => {
      ...
      t.forEach((state) => {
        ...
        const stateElectoralVotes =
          e.states_json[stateIndex].fields.electoral_votes;
  ...
  function setStatePollText(s, t) {
    ...
    if (!doPrimaryMode && !e.primary) {
      var c =
        "<h3>ESTIMATED SUPPORT</h3>                    <ul id='switchingEst'>" +
        _ +
      ...
      var c =
        "<h3>PRIMARY/CAUCUS RESULT</h3>                    <ul id='switchingEst'>" +
        _ +
        "</ul>                    <button id='pvswitcher' onclick='switchPV()'>PV Estimate</button><button onclick='evest()' id='ev_est'>Current Delegate Count</button>";
    }
    ...
    for (a = 0; a < e.final_overall_results.length; a++) {
      ...
      for (s = 0; s < e.final_overall_results.length; s++) {
        ...
        for (n = 0; n < l.length; n++)
          l[n].candidate == e.final_overall_results[s].candidate && (_ = 0);
        ...
        for (var s = 0; s < e.final_state_results.length; s++)
          if (e.final_state_results[s].abbr == a.name) {
            if (e.final_state_results[s].result_time > t) {
              var n =
                "                        <h3>STATE RESULTS</h3>                        <p>Returns for this state are not yet available!</p>";
              $("#state_result").html(n);
            } else {
              let o;
  ...
  function m() {
    if (e.primary) {
      ...
      for (i = 0; i < e.final_overall_results.length; i++) {
        ...
        e.final_overall_results[i].electoral_votes =
          filteredCandidates[trueIndex].evvs;
      }
    }
    for (
    ...
    var _ = [];
    for (a = 0; a < e.final_state_results.length; a++)
      for (var r = 0; r < e.final_state_results[a].result.length; r++)
        _.push({
  ...
  function p() {
    ...
    if (e.final_overall_results[0].electoral_votes >= a)
      var l = e.candidate_json[n].fields.image_url;
    else l = e.election_json[t].fields.no_electoral_majority_image;
    let o;
    for (o = 0, _ = 0; _ < e.final_overall_results.length; _++)
      o += e.final_overall_results[_].popular_votes;
    var r = "";
    ...
    for (_ = 0; _ < e.final_overall_results.length; _++) {
      ...
      r +=
        ...
        ';">----</span> ' +
        (f =
          e.candidate_json[i].fields.first_name +
          " " +
    ...
    if ("None" != e.game_results_url)
      var c =
        '<h4>Final Results: <a target="_blank" href="' +
        e.game_results_url +
    ...
    else if ("loss" == e.final_outcome)
      p = "I lost the " + v + " election as " + f + ". How would you do?";
    else if ("tie" == e.final_outcome)
      p =
        "I deadlocked the " +
        v +
  ...
  function h() {
    for (var t = f(500), i = u(), a = "", s = 0; s < i.length; s++) {
      for (var n = 0; n < e.final_overall_results.length; n++)
        if (e.final_overall_results[n].candidate == i[s].candidate)
          var l = e.final_overall_results[n].electoral_votes;
      a +=
        '            <li><span style="color:' +
        i[s].color +
        "; background-color: " +
    ...
    for (s = 0; s < e.final_overall_results.length; s++) {
      for (
        ...
        if (e.final_state_results[u].result[0].candidate == _) {
          var v =
            e.final_state_results[u].result[0].percent -
  ...
  function b() {
    S(e.election_id);
    for (var t = 0, i = 0; i < e.final_overall_results.length; i++)
      t += e.final_overall_results[i].popular_votes;
    var a = "";
    for (i = 0; i < e.final_overall_results.length; i++) {
    ...
    let o;
    if ("None" != e.percentile)
      var l =
        "<p>You have done better than approximately <strong>" +
        e.percentile +
    ...
    if ("None" != e.historical_overall) {
      ...
      for (i = 0; i < e.historical_overall.length; i++)
        o +=
          ...
          "</td></tr>";
      var _ =
        '<div id="overall_stat_details">            <h4>Historical Results - Your Candidate and Difficulty Level</h4>            <table>            <tr><th>Candidate</th>                <th>Win %</th>                <th>EV Avg.</th>                <th>PV Avg.</th>                <th>PV % Avg.</th>                <th>EV Range</th>                <th>PV Range</th>            </tr>' +
        o +
        "</table>            </div>";
    } else (o = ""), (_ = "");
  ...
  function k(e) {
    for (var t = "", i = 0; i < e.length; i++)
      t += '<option value="' + e[i].state + '">' + e[i].name + "</option>";
    return t;
  }
  ...
  function T(t) {
    for (
      var i =
          "        <h4>Results - This Game</h4>        <table>\t    <tr><th>Candidate</th><th>Popular Votes</th>\t    <th>Popular Vote %</th><th>Electoral Votes</th></tr>",
        a = 0;
  ...
  function A(t) {
    ...
    for (a = 0; a < y.length; a++) {
      ...
      if (
        ...
        "1" == e.game_type_id || "3" == e.game_type_id)
      )
    ...
    if (2 == t) {
      for (a = 0; a < y.length; a++) {
        ...
        for (x = 0, r = 0; r < y[a].result.length; r++)
          x += y[a].result[r].result;
        for (r = 0; r < y[a].result.length; r++) {
  ...
  vpSelect = (t) => {
    ...
    t.preventDefault(),
      (function (t, a) {
        ...
        $("#game_window").html(r),
          $("#running_mate_id").ready(function () {
            i();
          }),
      ...
      (function (a) {
        ...
        for (var n = "", l = 0; l < e.candidate_json.length; l++)
          e.candidate_json[l].fields.election == a &&
            1 == e.candidate_json[l].fields.is_active &&
            (n +=
              "<option value=" +
...
fix1964 = window.setInterval(function () {
  try {
    if (document.getElementById("election_id").value == 69) {
      ...
      windowT.children[windowT.children.length - 1].innerHTML =
        "This scenario was made by Tex.";
    } else if (document.getElementById("election_id").value > -1 && !modded) {
      windowT.children[windowT.children.length - 1].innerHTML =
        "This scenario was made by Dan Bryan.";
    }
  } catch {}
}, 100);
